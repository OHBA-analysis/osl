...
@author: Alexandre Gramfort @ Inria, CEA/Neurospin, Universite Paris-Saclay, Paris, France
        ...
        Adapted by Mark Woolrich @ OHBA, to do osl.rhino pipeline
...
def my_var_cut_fn(epochs, plow, phigh, to_plot=True):
    ...
    if to_plot:
        ...
        plt.ylim(min(trl_var) - min(trl_var) * 0.01, max(trl_var) + max(trl_var) * 0.01), plt.title(
...
events = mne.find_events(raw, stim_channel=par['stimch'], min_duration=0.001, shortest_event=1)
events = events[events[:, 1] == 0]
if more_plots:
    mne.viz.plot_events(events, first_samp=0, event_id=par['event_dict'], equal_spacing=True, show=True)
raw.pick_types(meg=True)

# %% Apply filter if required
raw.filter(2, 95, picks=None, filter_length='auto', l_trans_bandwidth='auto',
...
for stimcat in list(par['event_dict'].keys()):
    ...
    print('\n%d trial to remove from total %d trials...\nNo. of remaining trials = %d\n' % (len(bad_trials),
                                                                                            len(epochs_stimcat),
                                                                                            len(epochs_stimcat) - len(
                                                                                                bad_trials)))
    epochs_stimcat.drop(bad_trials, reason='variance based rejection', verbose=True)
    bad_trials = []

    # Compute covariance
    noise_cov = mne.compute_covariance(epochs_stimcat, tmin=par['ctrlwin'][0], tmax=par['ctrlwin'][1],
                                       method='empirical', verbose=True)
    data_cov = mne.compute_covariance(epochs_stimcat, tmin=par['actiwin'][0], tmax=par['actiwin'][1],
                                      method='empirical', verbose=True)
    ...
    cov_rank = None if epochs_stimcat.info['proc_history'] == [] else int(
