...
# parcellation to use
parcellation_fname = "fmri_d100_parcellation_with_PCC_reduced_2mm.nii.gz"
...
dataset = osl.preprocessing.run_proc_chain(
    fif_file_in, config, outdir=base_dir, overwrite=True
)
...
out_nii_fname = op.join(
    stats_dir, "acope{}_mni_parcel.nii.gz".format(contrast_of_interest, gridstep)
)
acope_nii_fname = p.nii(
    acope, method="assignments", times=epochs.times, out_nii_fname=out_nii_fname
)
rhino.fsleyes_overlay(parcellation_background_fname, acope_nii_fname)
